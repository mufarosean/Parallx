...
type NavKey = 'calendar' | 'tasks' | 'automations' | 'settings';
...
const ICONS: Record<NavKey, string> = {
  ...
  automations: '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>',
...
export class PlannerSidebar implements IDisposable {
  ...
  createView(container: HTMLElement): IDisposable {
    ...
    const rows: { key: NavKey; label: string; onClick: () => void }[] = [
      ...
      },
      {
        key: 'automations',
        label: 'Automations',
        onClick: () => void this._openTab('automations'),
      },
      {
        key: 'settings',
        label: 'Settings',
  ...
  private async _openTab(tab: 'calendar' | 'tasks' | 'automations'): Promise<void> {
    try {
      this._setActive(tab);
