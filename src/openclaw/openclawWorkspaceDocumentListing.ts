...
async function collectWorkspaceDocuments(
  listFiles: (relativePath: string) => Promise<readonly { name: string; type: 'file' | 'directory' }[]>,
  relativePath: string = '',
  depth: number = 0,
  results: string[] = [],
): Promise<string[]> {
...
export async function tryHandleWorkspaceDocumentListing(options: {
  ...
  readonly listFiles?: (relativePath: string) => Promise<readonly { name: string; type: 'file' | 'directory' }[]>;
