...
try {
  ...
  const revealAction = page.getByRole('button', { name: 'Show Answer', exact: true });
  const skipAction = page.getByRole('button', { name: 'Skip', exact: true });
  const revealRect = await revealAction.boundingBox(), skipRect = await skipAction.boundingBox();
  assert.equal(revealRect.width, skipRect.width, 'Reveal and Skip should have matching widths');
  assert.equal(revealRect.height, skipRect.height, 'Reveal and Skip should have matching heights');
  for (const action of [revealAction, skipAction]) {
    assert.equal(await action.innerText(), '', 'Study actions should be icon-only');
    assert.equal(await action.locator('svg').count(), 1);
    assert(await action.getAttribute('title'), 'Study actions need hover descriptions');
  }
  assert.equal(await page.locator('.fc-study__notes').isVisible(), false, 'Notes must not reveal the answer early');
  assert.equal(await page.locator('.fc-study__cardactions .fc-flag').count(),4);
  await page.getByRole('radio',{name:'Red flag',exact:true}).click();
  ...
  assert.equal(await page.locator('.fc-study__notes textarea').count(),0,'Empty notes should not show an editor');
  await page.getByRole('button',{name:'+ Add notes',exact:true}).click();
  const note = '**Key idea**\n\nContrast the underlying risk drivers, then explain why the segmentation differs.\n\n$$\\frac{a}{b}$$';
  await page.getByRole('textbox',{name:'Card notes',exact:true}).fill(note);
  await page.getByRole('button',{name:'Save notes',exact:true}).click();
  await page.waitForFunction(async note => {
    const result = await window.parallxElectron.extensionDatabase.get('flashcards', 'SELECT COUNT(*) AS n FROM fc_cards WHERE notes = ?', [note]);
    return result.row?.n === 1;
  }, note);
  await page.locator('.fc-study__notes-body .katex').first().waitFor();
  assert.equal(await page.locator('.fc-study__notes-body strong').innerText(),'Key idea');
  const notesBounds = await page.locator('.fc-study__notes').boundingBox();
  const answerBox = await page.locator('.fc-study__answer-host').boundingBox();
  assert(notesBounds.y >= answerBox.y + answerBox.height, 'Notes should follow the card, never sit beside it');
  const toolbarBox = await page.locator('.fc-study__toolbar').boundingBox();
  assert(Math.abs(toolbarBox.x-answerBox.x)<2 && Math.abs(toolbarBox.width-answerBox.width)<2,'Toolbar should align with the card');
  ...
  await page.evaluate(() => document.documentElement.removeAttribute('data-px-mode'));
  await page.locator('.fc-study__main').evaluate(e => { e.scrollTop = 0; });
  await shot('study-balanced', '.fc-study__main');
  await command('flashcards.customStudy');
  await page.locator('.fc-cs__mode').filter({hasText:'Extra New Cards'}).click();
  await page.getByRole('button',{name:/^Study \d+ Cards?$/}).click();
  await page.locator('.fc-study__front').waitFor();
  assert(await page.locator('.fc-study__toolbar .fc-study__mode').isVisible(), 'Custom mode should be inside the toolbar');
  await shot('study-custom-toolbar', '.fc-study__main');
