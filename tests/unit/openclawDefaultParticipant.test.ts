...
describe('openclaw default participant', () => {
  ...
  it('executes runtime-controlled tool calls inside the separate OpenClaw lane', async () => {
    ...
    expect(invokeToolWithRuntimeControl).toHaveBeenCalledWith('fs_read_file', { path: 'Policy.md' }, expect.objectContaining({ isCancellationRequested: false }), undefined, 'session-2', { resultCharBudget: expect.any(Number) });
