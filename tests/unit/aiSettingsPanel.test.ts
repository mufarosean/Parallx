...
describe('AISettingsPanel', () => {
  ...
  it('renders navigation with 6 section buttons', () => {
    const panel = new AISettingsPanel(parent, service as any);
    ...
    expect(navItems[4].textContent).toBe('Advanced');
    expect(navItems[5].textContent).toBe('Preview');
    panel.dispose();
  });
  ...
  it('renders all six sections in content area', () => {
    const panel = new AISettingsPanel(parent, service as any);
    ...
    expect(ids).toEqual(['persona', 'chat', 'suggestions', 'model', 'advanced', 'preview']);
    panel.dispose();
  });
