---
description: Parallx workspace agent rules. Body below is the verbatim Parallx Systems Redesign Manifest. Highest-level contract for any work in this workspace.
applyTo: '**'
---

# READ FIRST — STANDING CONTRACT (every turn, no exceptions)

You are on branch `systems-redesign-planning`. The manifest below is your contract.

1. **Iterate autonomously.** Do not wait for per-slice approval. The code drives the work, not milestone docs.
2. **Commit and push to this branch freely.** Do NOT touch `master`, delete branches, force-push, or rewrite history.
3. **Net positive only.** Aggressive improvement is encouraged. Break things only if the replacement is provably better and ships with a migration. Never leave a regression.
4. **AI chat infrastructure is off-limits** — `src/openclaw/**`, `src/services/chatAgentService.ts`, and the chat agent runtime that mimics OpenClaw. The API surfaces around it (workspace context, tools, retrieval, settings, language-model service) ARE in scope.
5. **§13a discipline for preservation surfaces.** Separate subagent for Executor and for Fitness-and-Review on `electron/**`, `src/built-in/**`, canvas registry/data/persistence, and other §11 surfaces. If subagent invocation fails, route around it (fresh-context re-read pass) — do not stop.
6. **Use "I", not "we".** No safety/hotline language. Direct and short.

Before producing a plan or diff this turn, confirm to yourself: which subsystem am I working in, what's the §16 work definition, what's the §11 preservation check, what's the rollback. If you cannot answer these, the right next action is to produce the atlas / baseline / work-definition for the target — not to ship code.

Full manifest follows.
...
6. Execute the plan continuously, slice by slice, without waiting for per-slice user approval, escalating only on destructive or out-of-scope actions defined in Â§18.

The agent should treat this manifest as the highest-level product contract for this redesign branch.

When this document says "accepted," it means one of: (a) the agent has produced the required artifact under the manifest's process and the Â§13a subagent review has returned a keep decision; or (b) explicit user approval for the narrow set of actions reserved to the user in Â§18. The agent is authorized to self-accept its own non-destructive artifacts and slices when the manifest's quality gates (Â§13a subagent role separation, Â§16 work definition, Â§22 verification) are met. The agent must NOT wait for user approval on routine slices.
...
Iterate continuously until Parallx is measurably better against Â§12 Definition Of Better. Each iteration:

1. Pick the next named target workflow from Â§5 / Â§6 or from the active milestone.
2. Produce the Â§16 Work Definition Contract for the slice (workflow, current behavior, pain, scope, baseline, better claim, preservation checks, verification, rollback).
3. Invoke a Surgical Executor subagent to implement the slice.
4. Invoke a separate Fitness and Review subagent to keep/revise/rollback (Â§13a â€” MUST be a different subagent session than the executor).
5. Run Â§22 verification.
6. Commit per Â§19 / Â§20.
7. Update the milestone doc.
8. Start the next iteration.

The agent does NOT stop and ask the user between slices. The agent stops only on the conditions in Â§18 Decision Rights: changing `master`, force-pushing or rewriting published history, deleting branches/checkpoints, breaking existing extension APIs/settings/keybindings/saved data without a net-positive replacement and migration, or accepting a regression. Scope is driven by the state of the codebase, not by a milestone doc.

### Autonomous Iteration Mandate

The agent is expected to run without sitting the user at the keyboard. The quality gates are subagent review (Â§13a), work-definition discipline (Â§16), and verification (Â§22) â€” all of which the agent runs itself. Lack of user approval on a routine slice is NOT a reason to stop. The only reasons to stop are:

- A Â§18 user-reserved decision is required.
- The Â§13a Fitness and Review subagent returns a rollback or revise decision the executor cannot resolve.
- Verification fails and cannot be made green within the slice.
- A preservation rule in Â§11 would be violated without a net-positive replacement.

Otherwise the agent keeps iterating. The agent reads the codebase, decides which subsystem is most broken or most leveraged next, and works there â€” milestone docs are the record of what was done, not the driver of what to do.
...
- **AI chat infrastructure** â€” the runtime that mimics OpenClaw (`src/openclaw/**`, `src/services/chatAgentService.ts`, the chat agent loop, the tool-call interpreter, the participant/skill plumbing). It will be revisited after the rest of the app is in top shape. The **surrounding API surfaces** (how chat consumes workspace context, resources, tools, settings, language-model service, retrieval) ARE in scope â€” only the core agent runtime is off-limits.
- Replacing Claude/OpenClaw behavior.
- **Breaking** existing extension APIs, settings, keybindings, or saved data without a provably better replacement and a migration path. ("Net positive only" â€” break only to improve.)
...
Â§13 and Â§14 imply this, but a previous slice (`7dca7c0e`, reverted in `68635c07`) shipped a regression because the conductor agent acted as Surgical Executor and Fitness and Review Agent in the same session. This section makes the rule explicit and enforceable.

Rules:

1. For any slice that touches a preservation surface listed in Â§11 or in `/memories/repo/orchestrator-discipline.md` (notably `src/built-in/*/main.ts`, `electron/**`, `src/openclaw/**`, `src/services/chatAgentService.ts`, `src/contributions/**`, `src/links/linkResolverService.ts`, and the canvas data/persistence/registry files), the conductor MUST invoke a separate subagent as Surgical Executor and a different subagent as Fitness and Review Agent. The conductor MUST NOT author the executor's diff and the reviewer's decision in the same agent session.
2. "Audit", "reality check", and "status" commits MUST include cited file:line diffs against current source for every claim they make. Narrative summaries without citations are forbidden. (Manifest Â§15.)
...
5. If the environment cannot invoke subagents on a given turn, the conductor MUST find another way to get independent review before closing the slice â€” a fresh-context re-read pass against the Â§16 contract and Â§11 preservation list, or a deferred review subagent invocation on the next turn. The conductor does NOT stop and wait for user approval; it routes around the failure. The commit body must record `single-pass-review: <method>` when no subagent reviewer was used.
6. A bug reaching the user after a slice closure is process failure by definition (Â§22). The next slice MUST add the gate that would have caught it.
...
| Whether a slice is ready for implementation | Systems Redesign Conductor after Â§16 work-definition + Â§11 preservation checks |
...
- Accepting a known regression as a tradeoff. (Aggressive improvement is encouraged â€” just keep the net positive.)
...
- Refactor any subsystem on the working branch when the Â§16 + Â§11 + Â§22 gates pass.
- Add, replace, or restructure any code outside the off-limits surfaces in Â§8, as long as the result is net positive.
...
| C4: App-system implementation | Atlas + baseline for the target subsystem | Implementation slices with Â§13a review and Â§22 verification. |

C4 work on a given subsystem should not start before C2/C3 exist for that subsystem â€” not because of a process gate, but because slicing without a map and a baseline is how regressions happen. The agent may run C0â€“C4 in parallel across different subsystems.
