...
// extensions are loaded via blob URL, and blob URLs cannot resolve relative
// imports — so a separate `./readability.js` is unreachable at runtime.
// The vendored Readability source remains in readability.js for source-of-
// truth / refresh diffing only; main.js is the single executable artifact.
...
function _registerNewsBriefWidget(api) {
  ...
  try {
    _commandDisposables.push(api.dashboard.registerWidgetType({
      ...
      description: 'A short AI-written summary of today\'s top news for your area, researched with webSearch and webFetch. Works for any location or beat (local news, an industry, a team).',
...
// ---------------------------------------------------------------------------
// SECTION 11 � Inlined Readability (vendored from readability.js)
// ---------------------------------------------------------------------------
//
// Source of truth: ./readability.js (kept on disk for diff/refresh). At
// runtime this file is loaded via blob URL which cannot resolve relative
// imports, so the vendored Readability body is appended here verbatim.
// To refresh: replace this section with the new readability.js body
// (without its trailing ESM export { Readability };).
