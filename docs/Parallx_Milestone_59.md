...
| **P3** | Search bar + smart albums + duplicate finder UI + perceptual hash | ✅ Landed |
| **P4** | Virtualized grid + background thumbnail queue indicator | ✅ Landed |
| **P5** | Stacks (versions) + soft-delete trash | ✅ Landed |
| **P6** | Timeline / calendar view + map view | ✅ Landed |
| **P7** | Drag from grid into chat + canvas, Reveal-in-MO from file tree | 🟡 Partial |
| **P8** | AI tools (image tagging, captioning, find-similar, smart-stack, auto-rate) | 🟡 Partial |
...
- **2026-04-29** — Phase 2 landed. Frame strip below clip dialog (30 sample frames @ 80×45 with overflow-x scroll), audio waveform via `showwavespic`, right-click delete, click → set per-frame delay (ms), reverse-frame-order checkbox, GIF concat-demuxer export path (re-extracts frames at user scale, drops deleted, reverses if checked, palette-based encoding). `mo_settings` k/v table + `media-organizer.setAutoWebPMode` command (Off/Suggest/Auto). Auto mode runs after scan completes — converts new animated WebP files to MP4 with quarantine. Manifest bumped to 0.4.0. Still deferred to later phases: F2.3 crop, F4.3-4.5 lightbox EXIF/rotation, ffmpeg progress streaming.
- **2026-04-29** — Phase 3 landed. FTS5 search (`mo_photos_fts` + `mo_videos_fts`, unicode61 tokenizer) over title/details/tags/folder, rebuilt post-scan. Search input now parses `tag:foo`, `-tag:bar`, `rating:>=4` / `rating:3..5`, `folder:beach`, `taken:2024-06`, `type:photo|video` with free text routed to FTS MATCH; results merged into existing query path via `idWhitelistPhotos/Videos` constraint in `applyFilterCriteria`. Perceptual hash dHash 64-bit (9×8 grayscale via ffmpeg, BigInt math, signed-int64 SQLite encoding) on `mo_image_files.phash`, runnable via `media-organizer.buildPHashes` with status-bar progress. Duplicate finder modal (`media-organizer.findDuplicates`) shows exact-match groups (md5 from `mo_fingerprints`) plus near-dup clusters (single-link Hamming ≤ 8); radio-keeper UI sends others to OS trash. Smart albums (`mo_smart_albums`, save/open/manage commands) snapshot the search input + filter state. Manifest bumped to 0.5.0 (5 new commands).
- **2026-04-29** — Phase 4 landed. IntersectionObserver-based lazy thumbnail resolution in `renderCardGrid` (rootMargin 400px, threshold 0.01) — DB queries fire only when cards near the viewport. `.mo-card` gets `content-visibility: auto` + `contain-intrinsic-size: 280px 240px` for browser-native off-screen virtualization. Per-page dropdown extended with 500/page and "Auto (large)" 2000-item modes; `resolvePagination` cap raised from 100 → 5000. F7.3 thumbnail queue indicator was already present in `generateAllThumbnails` via `_statusBarItem`.
- **2026-04-29** — Phase 5 landed. Migrations 015 (stacks: `mo_stacks(id, primary_type, primary_id, name)` + `mo_stack_members(stack_id, member_type, member_id, role, position)`) and 016 (`mo_photos.deleted_at` + `mo_videos.deleted_at` — diverged from doc which suggested `mo_files.deleted_at`; rationale documented in migration). `buildUnifiedQuery` and `buildSingleTypeQuery` now exclude trashed items by default and hide non-primary stack members via `NOT EXISTS` clauses; trash view (`filterType === 'trash'`) inverts that filter. New sidebar "Trash" item, stack badge `+N` on cards (lazily fetched alongside thumbnails), and 4 commands: `stackSelected`, `autoStack` (groups by normalized basename within folder), `moveToTrash`, `emptyTrash`. Auto-purge for trash entries older than `mo_settings.trash_auto_purge_days` (default 30) runs on activation. Manifest 0.6.0.
