...
| **5.2** | **End-to-end smoke test** | ✅ | 30m | 5.1 | — | Smoke test checklist created in `demo-workspace/SMOKE_TEST_CHECKLIST.md` with 6 scenarios: situational (fender bender), direct question, greeting, follow-up, exploration, empty workspace fallback. Unit tests (34 M12 + full suite 1296/1296) all pass. Manual E2E testing to be done at demo time with Ollama running. |
| **5.3** | **Demo workspace preparation** | ✅ | 30m | 5.2 | — | Created `demo-workspace/` with 5 markdown files: (1) Auto Insurance Policy (coverage types, limits, $500/$250 deductibles), (2) Claims Guide (step-by-step procedure, deadlines, UM claims), (3) Agent Contacts (Sarah Chen, phone, office hours, preferred shops), (4) Vehicle Info (2024 Honda Accord, VIN, maintenance), (5) Accident Quick Reference (glove-box card). All `.md` files are indexable by the pipeline. |
