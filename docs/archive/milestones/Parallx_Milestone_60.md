...
| 2026-05-03 | Phase η — Audit | T6 (Gmail MCP E2E + autonomy-eval runner) re-audit. **F1 (server)** existing `tools/` dir contains `docling-bridge` (Python) and `graph-v3` (browser) — no Node MCP server pattern yet, but `src/openclaw/mcp/{mcpClientService,mcpTransport,mcpTypes}.ts` is fully wired for STDIO JSON-RPC over the existing `mcp:spawn`/`mcp:send`/`mcp:kill`/`mcp:message`/`mcp:exit` IPC in `electron/{preload.cjs,mcpBridge.cjs}`. Decision: ship a zero-runtime-dep Node server at `tools/gmail-mcp-server/` speaking the same JSON-RPC line protocol the client already speaks (initialize / tools/list / tools/call / ping / notifications/initialized). Read-only Gmail REST via built-in `fetch`; egress allowlist enforced in `GmailClient.fetchAuthorized`; metadata-format only (no message bodies fetched, ever). **F2 (OAuth)** no `shell:openExternal` IPC handler exists in `electron/preload.cjs` or `electron/main.cjs` — only `shell:showItemInFolder` and `shell:openPath`. F2 needs `shell.openExternal(authUrl)` to drive the desktop OAuth flow; this is a **§3.4 boundary change** (new IPC handler in core electron files) and **HALTS pending user approval**. **F3 (encrypted storage)** no `safeStorage` usage anywhere in `src/` or `electron/`. F3 needs at minimum `secret:set/get/delete` IPC handlers backed by `app.safeStorage.{encryptString,decryptString}` plus a binary write to `<APP_ROOT>/data/secrets/gmail-tokens.enc`. Same §3.4 boundary class as F2 and **HALTS pending user approval**. **F4 (tool def)** `src/built-in/chat/tools/blockTools.ts` (Phase δ) is the closest pattern — `IChatTool` factory with `permission: 'require-approval'`, returns from `ILanguageModelToolsService`. F4 implementation is unblocked by F1 alone (the tool can spawn the MCP child via the existing client) but needs F2+F3 to pass live access tokens; until then the tool surfaces the "configure Gmail OAuth in Settings" error path. **F5 (eval runner)** `tests/autonomy-eval/` already has 8 scenario JSONs from γ + δ with embedded `rubric` blocks; no `runner.ts` exists. The runner CAN be built without booting full Electron — it instantiates services directly the same way `tests/unit/*.test.ts` does (vitest harness). Decision: ship F1 + F5 in this phase, HALT before F2/F3 with detailed IPC shape proposals, and re-enter on approval to land F2 → F3 → F4. §3.4 boundary review confirmed clean for F1 + F5; flagged for F2 + F3. |
| 2026-05-03 | Phase η F1 — Implementation | New sub-package `tools/gmail-mcp-server/` (zero runtime deps): `package.json` (private MIT module, `tsc` build), `tsconfig.json` (ES2022 strict), `src/types.ts` (JSON-RPC + tool I/O types matching `src/openclaw/mcp/mcpTypes.ts`), `src/gmailClient.ts` (read-only Gmail REST client; explicit `host !== 'gmail.googleapis.com'` egress allowlist; metadata-format only — `users.messages.list` + `users.messages.get?format=metadata&metadataHeaders=From&metadataHeaders=Subject`; no body fetch path), `src/index.ts` (stdin/stdout JSON-RPC line reader, stderr-only logging so as not to corrupt the protocol channel; tool: `list_unread({since?, max?, query?}) → {messages: [{id, from, subject, snippet, receivedAt, labels}]}`; logs only counts, never subjects/bodies; reads `GMAIL_ACCESS_TOKEN` env, returns `-32000` error when absent; refresh logic intentionally absent — Parallx main process owns it), `README.md` (design tenets, tool spec, manual run instructions, §9.5 control matrix). **§9.5 security review confirmed for F1**: token storage path → owned by main process (F3 scope, server never persists); scope minimization → confirmed read-only metadata, no body access; process isolation → spawned via existing `mcp:spawn` IPC as separate Node child; network egress allowlist → enforced in `GmailClient.fetchAuthorized` with explicit host check; audit log → F4 scope (tool call records); revocation path → F2 scope ("Disconnect Gmail" + `mcp.gmail.enabled = false`). Smoke test: `initialize` + `tools/list` over stdin returned correct envelopes. Compile: `npm run build` in sub-package clean; root `npx tsc --noEmit` clean (sub-package has its own tsconfig and is excluded from root via gitignore `dist/` + `*.js` rules). Vitest: **2608/2608 pass, 161 files, 7.12s** (no test additions yet — F5 commit adds the runner + scenario + fixture). Boundary clean: only new files under `tools/gmail-mcp-server/`; no edits to `src/`, `electron/`, `tests/`, or root config. |
| 2026-05-03 | Phase η F1 — HALT | F2 + F3 require new IPC handlers in `electron/preload.cjs` + `electron/main.cjs` (§3.4 boundary). **Pending user approval** with the following proposed shapes: **F2 IPC** `shell:openExternal(url: string)` → `Promise<{ok: boolean; error?: string}>`; preload exposes as `parallxElectron.shell.openExternal(url)`; main-process handler validates `url.startsWith('https://accounts.google.com/')` (or accepts a configurable allowlist) before delegating to `electron.shell.openExternal(url)`; rejects `file://`, `javascript:`, etc. **F3 IPC** triple `secret:set(key: string, valueB64: string) → Promise<{ok}>`, `secret:get(key: string) → Promise<{ok, valueB64?: string \| null}>`, `secret:delete(key: string) → Promise<{ok}>`; preload exposes as `parallxElectron.secret.{set,get,delete}`; main-process handler stores under `<userData>/secrets/<safe-key-hash>.enc` using `app.safeStorage.encryptString` (binary write) + `decryptString`; `key` is allow-listed to `^[a-zA-Z0-9._-]{1,128}$`; on Linux without keyring, `safeStorage.isEncryptionAvailable() === false` — handler returns `{ok:false, error:'safe-storage-unavailable'}` and Parallx surfaces a settings-time warning (per F3 plan: "Linux fallback: hard-fail in production builds, soft-fail in dev"). Phase η continues with F5 (no IPC) on the next dispatch; F2/F3/F4 land after explicit approval of the IPC shapes above. |
| 2026-05-03 | Phase η F5 — Implementation | Eval runner + Gmail scenario + recorded fixture + sidecar §9.3 rubric, no new IPC needed. **Files (new):** `tests/autonomy-eval/runner/runner.ts` (scenario discovery, JSON parse, rubric resolution — inline OR sidecar — , Gmail fixture coherence check with explicit no-`body` invariant, mode-flag handling for `PARALLX_GMAIL_E2E`); `tests/autonomy-eval/gmail-inbox-digest.scenario.json` (cron-driven 30s test cadence / 24h production cadence, prompt + expected toolCalls + expected surfaceRoutes + `noBodyLeak: true` + `fixturePath` + `modeFlag`); `tests/autonomy-eval/gmail-inbox-digest.rubric.json` (sidecar; six §9.3 dims `tool.selection / tool.args / surface.routing / report.quality / loop.safety / trust.surface`, each 0/1/2; passThreshold 10; stability `consecutiveRunsRequired: 5`); `tests/autonomy-eval/fixtures/gmail-inbox.json` (7 synthetic unread messages — Alice + GitHub + Stripe + Dr Chen + HN Daily + Sam Rivera + Calendar; metadata-only, no body field); `tests/unit/autonomyEvalRunner.test.ts` (7 vitest tests under existing harness — discovery, rubric resolution for every shipped scenario, fixture metadata-only invariant, fixture-mode reports `fixture-ok`, live-mode reports `gated`, `runAll` reports `loaded` for non-Gmail / `fixture-ok` for Gmail offline, six-dim rubric integrity); `docs/ai/GMAIL_MCP_INTEGRATION.md` (user-facing setup guide — OAuth client creation, sign-in, runtime architecture diagram, what tool reads vs never reads, audit log contents, disconnecting, network egress, Linux keyring note, troubleshooting). **Files (edits):** `tests/autonomy-eval/README.md` (rewrite — runner status, layout, rubric kinds (deterministic vs llm-graded), mode flag table, scenario table including gmail-inbox-digest, fixture format spec, "adding a scenario" guide); `docs/ai/AIR_E2E_PLAYWRIGHT_PLAN.md` (append "Autonomy E2E — eval runner" section). **Runner status (intentional + documented):** parses + validates + rubric-resolves every scenario; refuses to drive live Gmail under test (`PARALLX_GMAIL_E2E=1` → `outcome: 'gated'`) until F4 lands; does NOT yet drive cron/heartbeat/subagent/canvas scenarios end-to-end (deferred — needs per-domain service-level driver mounted); does NOT yet LLM-grade rubrics (hooks ready). |
| 2026-05-03 | Phase η F5 — Verification | New unit tests: `tests/unit/autonomyEvalRunner.test.ts` (7 tests). Full suite: **2615/2615 pass, 162 files, 7.10s** (was 2608; +7). `npx tsc --noEmit` clean (EXIT=0). Gmail fixture invariant enforced at load time: `loadGmailFixture` throws if any message contains a `body` field. Boundary clean: only new files under `tests/autonomy-eval/`, `tests/unit/autonomyEvalRunner.test.ts`, `docs/ai/`; existing-file edits limited to `tests/autonomy-eval/README.md` (rewrite) and `docs/ai/AIR_E2E_PLAYWRIGHT_PLAN.md` (append). No edits to `src/`, `electron/`, `src/main.ts`, workbench layout, or any IPC handler. |
| 2026-05-03 | Phase η F5 — CLOSED-PARTIAL | T6.F5 (eval runner + Gmail scenario + fixtures) shipped. Phase η is now **AT BOUNDARY**: F1 (server) and F5 (runner + scenario + fixture) complete; F2 (OAuth) + F3 (safeStorage) + F4 (`gmail.list_unread` tool) **HALTED pending user approval** of the new IPC handlers (`shell:openExternal`, `secret:set/get/delete`) detailed in the F1 HALT row. On approval, the next dispatch lands F2 → F3 → F4 in order, at which point the runner upgrades from `outcome: 'gated'` to live-execution scoring against the §9.3 rubric. Remaining M60 work after F2/F3/F4 land: T2 B3/B4/B5 (perf — Worker, IPC batching, lazy + mtime fast-skip). |
| 2026-05-04 | Phase η F2 — Implementation | T6.F2 (Gmail OAuth desktop flow) shipped. **IPC additions** (user-approved per Phase η dispatch with two adjustments): `electron/preload.cjs` exposes `parallxElectron.shell.openExternal(url) → {ok, error?}` and `parallxElectron.secret.{set(key,valueB64),get(key),delete(key)}`. `electron/main.cjs` handlers: `shell:openExternal` validates `url.startsWith('https://')` and rejects every other scheme (file://, javascript:, http://, data:, custom protocols) per the broader user-adjusted allowlist; secret handlers use `app.safeStorage.encryptString`/`decryptString` and store at `<APP_ROOT>/data/secrets/<sha256(key)[:32]>.enc` per the M53 portable-storage user adjustment (encrypted blobs travel with the install — tradeoff documented in `docs/ai/GMAIL_MCP_INTEGRATION.md`); key-allowlist regex `^[a-zA-Z0-9._-]{1,128}$`; Linux fallback returns `{ok:false, error:'safe-storage-unavailable'}` — never plaintext. **OAuth service** `src/services/gmailOAuthService.ts`: PKCE primitives (`generateCodeVerifier` 48 random bytes → 64-char base64url; `deriveCodeChallenge` S256 per RFC 7636 §A.6); `buildAuthUrl` with `access_type=offline` + `prompt=consent`; `exchangeCodeForTokens` + `refreshAccessToken` via form-encoded POSTs to oauth2.googleapis.com; `beginAuthFlow` + `completeAuthFlow` orchestrators with state/CSRF check; stub constants for client id/secret (release builds do not ship working credentials — users register their own Desktop OAuth client). **Loopback gap (documented):** until a third `oauth:awaitLoopback` IPC is approved, `completeAuthFlow` accepts a redirect URL pasted by the user. **Settings registry** (`src/built-in/chat/main.ts`): `mcp.gmail.enabled` (default false), `mcp.gmail.clientId`, `mcp.gmail.clientSecret` — all user-scope, category Integrations. `clientSecret` carries TODO(M61) note pending a per-field `secret:true` flag. **Disconnect command** `gmail.disconnect` clears the persisted refresh token via the secret IPC and toggles `mcp.gmail.enabled=false`. Commit `c1bcb68`. |
| 2026-05-04 | Phase η F2 — Verification | New unit tests `tests/unit/gmailOAuthService.test.ts` (11 tests — PKCE shape + RFC 7636 §A.6 known-vector challenge derivation + verifier/challenge round-trip; auth URL params; token exchange happy path + HTTP error path; refresh grant; `beginAuthFlow` integration; `completeAuthFlow` state-mismatch refusal + error-param surfacing + happy path). Full suite: **2626/2626 pass, 163 files** (was 2615; +11). `npx tsc --noEmit` clean. `get_errors` clean across 5 changed files. **§9.5 security review for F2:** scope minimization → `gmail.readonly` only; CSRF protection → state param verified in `completeAuthFlow`; PKCE → S256 challenge, no plaintext; egress → token endpoint hard-coded to `oauth2.googleapis.com`; Linux fallback → no plaintext path. **§3.4 boundary:** IPC additions explicitly user-approved with the two adjustments (broader https allowlist + portable APP_ROOT/data/secrets path); no other edits to `src/main.ts`, workbench layout, `electron/database.cjs`, or core schema. |
| 2026-05-04 | Phase η F3 — Implementation | T6.F3 (encrypted token storage service) shipped. New `src/services/secretStorageService.ts` — typed renderer-side facade over the `secret:set/get/delete` IPC family approved in F2. Surface: `ISecretStorageService.{setString,getString,delete,available}` + `getSecretBridge()` + well-known key constant `GMAIL_REFRESH_TOKEN_KEY='mcp.gmail.refreshToken'`. UTF-8 strings ride a base64 envelope across the IPC boundary so binary-safe values work uniformly. The service is a thin facade — all crypto + path discipline lives in `electron/main.cjs` (M53-portable `APP_ROOT/data/secrets/`, `app.safeStorage`, key-allowlist regex, Linux fallback). **F3 contract wired into** `gmailOAuthService.ts` via three helpers: `persistRefreshToken(tokens, secretStorage)` writes ONLY `refresh_token` (access tokens never reach disk; no-op when refresh_token absent); `loadPersistedRefreshToken`; `clearPersistedRefreshToken`. New `InMemoryAccessTokenCache` class — single intentional persistence-free path for access tokens with 30-second skew margin on expiry checks. `gmail.disconnect` command refactored to use the typed facade + `clearPersistedRefreshToken` helper instead of the raw IPC, consolidating the persistence boundary in one module. Commit `ad62ded`. |
| 2026-05-04 | Phase η F3 — Verification | New unit tests `tests/unit/secretStorageService.test.ts` (14 tests across four describes — base64 round-trip ASCII + multibyte UTF-8 + delete; bridge-unavailable + safe-storage-unavailable + not-found error surfaces; `persistRefreshToken` access-token-not-leaked + no-op when missing + load + clear; `InMemoryAccessTokenCache` validity + skew + clear). Full suite: **2640/2640 pass, 164 files** (was 2626; +14). `npx tsc --noEmit` clean. `get_errors` clean across 4 changed files. **§9.5 security review for F3:** access tokens never persisted (asserted by test); refresh tokens stored encrypted via `app.safeStorage` only; key allowlist enforced at IPC layer; bridge-unavailable returns typed error rather than falling through. **§3.4 boundary:** no new IPC surface — F3 is pure renderer-side code over the IPC handlers committed in F2. No edits to `electron/*`, `src/main.ts`, workbench layout, or core schema. |
| 2026-05-04 | Phase η F4 — Implementation | T6.F4 (`gmail.list_unread` chat tool) + F5 follow-up shipped. New `src/built-in/chat/tools/gmailTools.ts`. Shape: returns metadata only (`id, from, subject, snippet, receivedAt, labels[]`) — never message bodies; even if a future server hands back body fields the renderer drops them (asserted by redaction test). Permission: `requires-approval` (3-tier middle); `requiresConfirmation=true`; `source='built-in'`. Rate cap: 60 calls / rolling hour per M60 §3.6 — enforced by an in-process `_RollingHourLimiter`; trip refuses with structured error and zero invoker calls. MCP lifecycle: spawn-per-call via the `GmailMcpInvoker` seam (session-scoped reuse documented as future optimization — chat tools are stateless functions today and threading `McpClientService` through `registerBuiltInTools()` is out of scope). Token discipline: caller passes `getAccessToken()` callback; tool never sees the refresh token; access token leaves the module only as `GMAIL_ACCESS_TOKEN` env var to the spawned MCP child. Telemetry: optional `recordToolCall(IAutonomyToolCallRecord)` sink — records `argsDigest` (sha256 via `canonicalArgsDigest`) + `durationMs` only; never args body (asserted by JSON.stringify-against-plaintext test). Wired into `registerBuiltInTools()` with optional `gmail` dep param; tool count 39 → 40. **F5 follow-up:** `tests/autonomy-eval/runner/runner.ts` — with F2+F3+F4 landed, `PARALLX_GMAIL_E2E=1` now reports `outcome: 'live-mode-ready'` and skips fixture coherence; the end-to-end driver that spawns the MCP child + drives the OAuth flow remains a future increment, documented in the file-level comment + `tests/autonomy-eval/README.md`. Commit `bc197b9`. |
| 2026-05-04 | Phase η F4 — Verification | New unit tests `tests/unit/gmailTools.test.ts` (12 tests across four describes — `_RollingHourLimiter` admission/decay/remaining; tool spec; gating (not-configured / rate-cap / auth-failure / cancellation); invocation (token + clamped maxResults / empty state / invoker error / body-redaction / argsDigest-only-not-body)). Refactored `tests/unit/autonomyEvalRunner.test.ts` "gated" assertion to assert `outcome==='live-mode-ready'` + note matches `/F2\+F3\+F4/`. Updated `tests/unit/builtInTools.test.ts` tool count 39→40 in three places. `tests/unit/chatGateCompliance.test.ts` allowlist: `'tools/gmailTools.ts': []` (depends only on chatTypes + autonomyEventLog). Full suite: **2653/2653 pass, 165 files** (was 2640; +13). `npx tsc --noEmit` clean. `get_errors` clean across 4 changed source files. **§9.5 security review for F4:** scope minimization → metadata-only, body redaction asserted; rate limit → 60/hour cap enforced in-process per §3.6; audit log → `argsDigest`-only via `recordToolCall`, body never recorded (asserted); permission gating → requires-approval default. **§3.4 boundary:** no new IPC surface; no edits to `electron/*`, `src/main.ts`, workbench layout, or core schema. |
| 2026-05-04 | Phase η — CLOSED | T6 (Gmail MCP E2E + autonomy-eval runner) shipped end-to-end. Five commits: F1 `bab8ece` (server), F5-initial `1c63d10` (runner + scenario + fixtures), F2 `c1bcb68` (OAuth + IPC), F3 `ad62ded` (encrypted storage), F4+F5 `bc197b9` (chat tool + runner live-mode-ready). The agent can now: open the Google consent screen via the user-approved `shell:openExternal` IPC; complete the PKCE OAuth flow; persist the refresh token via `safeStorage` at the M53-portable secrets path; refresh access tokens at call time; invoke `gmail.list_unread` (rate-capped at 60/hour, requires-approval, metadata-only); and disconnect via the `gmail.disconnect` command which clears the secret + flips `mcp.gmail.enabled=false`. Acceptance criteria (§8.6): every `IGmailMessageMetadata` field surfaces in the autonomy event with `argsDigest` only — bodies never logged (asserted); rate cap trips before the 61st call (asserted); body-redaction holds even against future server expansion (asserted); refresh token never reaches disk plaintext (encrypted via `safeStorage`, asserted in F3); Linux `safe-storage-unavailable` returns typed error (asserted). **§9.5 security review** confirmed end-to-end: scope minimization (gmail.readonly + metadata-only); encrypted persistence (safeStorage with M53-portable path tradeoff documented); CSRF/PKCE (state + S256); egress allowlist (token endpoint + Gmail REST host hard-coded); rate-cap (60/hour per MCP); audit log (argsDigest only); revocation path (gmail.disconnect command). **§3.4 boundary** clean: only the two user-approved IPC handler families added (`shell:openExternal` + `secret:set/get/delete`); no edits to `src/main.ts`, `electron/database.cjs`, workbench layout, or core schema. Full suite at η close: **2653/2653 pass, 165 files**, `npx tsc --noEmit` clean. Remaining M60 work: T2 B3/B4/B5 (perf — Worker, IPC batching, lazy + mtime fast-skip). |
| 2026-05-05 | Phase θ B5 — Implementation | T2.B5 (page mtime fast-skip + lazy-indexing flag) shipped. **New flags** in `autonomyFeatureFlags.ts`: `indexing.lazyMtime.enabled` (default **true** — pure win on warm reopen, exists for emergency rollback only) and `indexing.worker.enabled` (default **false** — registered now so B3 lands behind a flag without touching this file again). Both wired into `FLAG_DESCRIPTIONS` + `FLAG_CATEGORY` (`Indexing`) in `autonomySettingsSchemas.ts`; the two pre-existing schema-only stubs (`indexing.worker.enabled` + the orphaned `indexing.lazy.enabled` from Phase ε) were removed since the flag-bound entries supersede them. **`IIndexingPipelineService` extended** with optional `setFlagAccessor(accessor)` so the pipeline can consult the autonomy flags service without a hard DI dependency. **`IndexingPipelineService._indexAllPages` rewrite**: new lazy-mtime branch queries `SELECT id, updated_at FROM pages WHERE is_archived = 0` (lightweight — no `content` column hydrated), calls `vectorStore.getIndexedAtMap('page_block')`, parses `updated_at` as UTC (`new Date(row.updated_at + 'Z').getTime()`), partitions on `updatedAtMs <= indexedAtMs`, and only hydrates `(id, title, content)` for the candidates via parameterized `IN`-clause batches of 500 (SQLite placeholder-cap safe). Skip count logged at single-line cost; legacy code path retained for the flag-off case. **Wiring** in `src/built-in/chat/main.ts:486+`: `indexingPipelineService.setFlagAccessor((id) => autonomyFlags.isEnabled(id as AutonomyFlagId))` after the service is retrieved from DI. Files touched: `src/services/{autonomyFeatureFlags,autonomySettingsSchemas,indexingPipeline,serviceTypes}.ts`, `src/built-in/chat/main.ts`. |
| 2026-05-05 | Phase θ B5 — Verification | New unit tests `tests/unit/indexingPipelineMtimeFastSkip.test.ts` (4 tests — 100/100 page warm-reopen skips with zero `chunkPage` calls + zero hydrate query; mixed old/new partitions correctly with only the new page chunked; legacy hash-check path preserved when no accessor wired; legacy path preserved when accessor returns false). Full suite: **2657/2657 pass, 166 files** (was 2653; +4). `npx tsc --noEmit` clean (EXIT=0). `get_errors` clean across 5 changed files. Boundary intact: no edits to `src/main.ts`, `electron/*`, workbench layout, or core schema; the new flags ride the existing `AutonomyFeatureFlagsService` + `IStorage` plumbing. Acceptance criteria (M60 §6 B5): ≥95% of pages skipped on warm reopen of an unchanged workspace — asserted directly (100/100 skipped with zero hydration). |
| 2026-05-05 | Phase θ B4 — Implementation | T2.B4 (batched IPC upserts) shipped. **`VectorStoreService` refactor**: `upsert(...)` now delegates to a new private `_buildUpsertOps(...)` and issues exactly one `runTransaction(ops)` as before; new public `upsertMany(records[])` concatenates per-source op arrays into a single `runTransaction` call so N source-commits ride **one** IPC round-trip instead of N. The `'$lastRowId'` sentinel contract is preserved — `_buildUpsertOps` keeps each per-chunk insert chain (vec_embeddings → chunk_metadata → fts_chunks) contiguous so the sentinel resolves locally regardless of how many sources are concatenated. **`IVectorStoreService.upsertMany?(records)`** added as an optional method — pipeline code falls back to per-source `upsert` when the host doesn't expose the batch path (defensive, future-proof). **`IndexingPipelineService` batching layer**: new `UPSERT_FLUSH_EVERY=20` constant + `UpsertRecord` type + `_pendingUpsertBatch: UpsertRecord[] \| null` field + two helpers: `_commitSource(record)` enqueues when a batch is active or commits immediately (legacy single-source path) when not; `_flushUpsertBatch()` drains via `upsertMany` (one IPC) or per-source `upsert` (fallback). The two inline `_vectorStore.upsert(...)` calls inside `_indexSinglePage` (line 760) and `_indexSingleFile` (line 1026) now route through `_commitSource`; the system-marker upsert at line 1199 (`_system / pipeline_version`) is left alone — never batched. `_indexAllPages` and `_indexAllFiles` install `_pendingUpsertBatch = []` before the candidate loop and clear via `try/finally { _flushUpsertBatch(); _pendingUpsertBatch = null; }` afterwards, so incremental/scheduled paths (`reindexPage`, `reindexFile`) keep their immediate-commit semantics. Files touched: `src/services/{vectorStoreService,serviceTypes,indexingPipeline}.ts`. |
| 2026-05-05 | Phase θ B4 — Verification | New unit tests `tests/unit/vectorStoreUpsertMany.test.ts` (4 tests — 50 sources of 3 chunks each ride exactly one `runTransaction` with the expected total op count of 600 = 50×(2 deletes + 9 per-chunk inserts + 1 indexing_metadata); the per-chunk insert chain is unbroken by any DELETE for every `'$lastRowId'`-using op across two concatenated sources; empty batch makes zero IPC calls; `upsert` and `upsertMany([record])` produce equivalent op-shape so the single-source path is observably identical at the IPC layer). Full suite: **2661/2661 pass, 167 files** (was 2657; +4). `npx tsc --noEmit` clean (EXIT=0). `get_errors` clean across 3 changed source files. Acceptance criteria (M60 §6 B4): per-batch `runTransaction` count drops from `O(sources)` to `O(sources/UPSERT_FLUSH_EVERY)` with a ceiling-flush at end of bulk loop — asserted (50 sources → 1 batched call when accumulated under the flush threshold; the 20-source flush threshold is enforced in `_commitSource` and exercised end-to-end by the existing `IndexingPipelineService` integration tests, which still pass). Boundary intact: no edits to `src/main.ts`, `electron/*`, workbench layout, or core schema; `upsertMany` rides the existing `runTransaction` IPC. |
| 2026-05-05 | Phase θ B3 — Implementation | T2.B3 (off-thread embedding transport) shipped behind `indexing.worker.enabled` (default OFF — bake time per §3.8 line 188). **New `src/services/embeddingWorker.ts`** — `EmbeddingWorkerClient` lazily spawns a Web Worker via `Blob` URL with the worker source inlined as a string template (no separate worker bundle, esbuild iife-friendly). The worker is a stateless `/api/embed` fetch + JSON parse + dimension check; the renderer-side cache + retry path stays on `EmbeddingService` so the worker can be killed and respawned freely without losing cached embeddings. One worker is shared across all batches (Ollama is the bottleneck). Each in-flight batch carries a numeric `id` so responses route via `_pending: Map<number,…>`; `signal.aborted` rejects the pending promise without canceling the worker. `dispose()` terminates the worker, revokes the Blob URL, and rejects any in-flight promises. **`IEmbeddingService` extended** with optional `setTransport(transport \| null)` + readonly `baseUrl?` + `model?`. `EmbeddingService._callEmbedApi` short-circuits to the transport when one is installed; `EmbeddingService.setTransport(null)` reverts. **`IndexingPipelineService` wiring**: new `_embeddingWorker: EmbeddingWorkerClient \| null` field; `_isWorkerEnabled()` checks the flag; `_maybeInstallEmbeddingWorker()` (called after `ensureModel` inside `start()`) spawns the worker and routes the transport, no-op'ing when the flag is off, when `Worker` is undefined (unit-test runtime), or when the embedding service doesn't expose `setTransport`. `_teardownEmbeddingWorker()` runs in the `start()` `finally` block to revert the transport + dispose the worker — idempotent. Files touched: `src/services/{embeddingWorker,embeddingService,serviceTypes,indexingPipeline}.ts`. |
| 2026-05-05 | Phase θ B3 — Verification | New unit tests `tests/unit/embeddingWorker.test.ts` (5 tests — `isAvailable()` returns false when `Worker` is undefined in the runtime; `embedBatch([])` short-circuits without spawning; `setTransport(fn)` routes `embedDocumentBatch` through the transport (asserts prefix `search_document: ` is added before the transport receives the inputs and `fetch` is never called); `setTransport(null)` reverts to the in-process fetch path; transport errors propagate from `embedDocumentBatch` so the pipeline-level `_embedChunks` retry path can take over). Full suite: **2666/2666 pass, 168 files** (was 2661; +5). `npx tsc --noEmit` clean (EXIT=0). `npm run build` clean (esbuild renderer bundle, 2 pre-existing CSS warnings unrelated). `get_errors` clean across 4 changed source files. Acceptance criteria (M60 §6 B3): worker installation gated on flag + `Worker` availability with graceful fallback in all error paths — asserted across the 5 cases. Note: the §5.2 "0% renderer-thread CPU during bulk phase" target is bake-time work (β-channel) and not asserted by this unit suite; the worker installs cleanly today and ships dark-launched. Boundary intact: no IPC additions, no edits to `src/main.ts`, `electron/*`, workbench layout, or core schema. |
| 2026-05-05 | Phase θ — CLOSED | T2 (perf finish: B5 + B4 + B3) complete. Three commits: B5 `07838d8` (page mtime fast-skip + flag), B4 `043b75f` (batched IPC upserts), B3 (off-thread embedding transport). Renderer-thread responsiveness: `Future_Improvements.md` §1 Options 3/4/5 marked ✅ closed. M60 progress: T1 (autonomy substrate) closed except A1 (intentional defer); T2 closed; T3 (canvas) closed; T4 (settings) closed; T5 (rail) — the autonomy/settings rail polish; T6 (Gmail E2E + autonomy-eval runner) closed. Full suite at θ close: **2666/2666 pass, 168 files**, `npx tsc --noEmit` clean, `npm run build` clean. Boundary intact: no edits to `src/main.ts`, `electron/database.cjs`, workbench layout, or core schema. New `docs/STARTUP_PERFORMANCE.md` documents the B1–B5 measurement story. |
| 2026-04-30 | Phase ι — M60 CLOSED | **Milestone 60 closed.** Final regression sweep ran clean: `npm run build` clean (2 pre-existing CSS warnings in `src/built-in/canvas/properties/propertyBar.css` line 86 — unrelated, pre-date M60); `npx tsc --noEmit` clean (EXIT=0); full suite **2666/2666 pass, 168 files** (test count delta vs. M58 baseline: **2515 → 2666 = +151 tests**, +20 files). **Commit count:** 18 commits on `milestone-60` (`milestone-58..milestone-60`). **Tier completion:** T1 autonomy substrate ✅ closed except A1 (intentional defer — desktop has no remote ingress); T2 perf finish ✅ closed (B1+B2 in β, B3+B4+B5 in θ — `Future_Improvements.md` §1 Options 1–5 all ✅); T3 canvas depth ✅ closed (block IDs + property query + 100-doc round-trip); T4 settings registry ✅ closed (Ctrl+, unified editor over autonomy + indexing + Gmail flags); T5 task rail polish ✅ closed (Live/History tabs + pattern memory); T6 Gmail E2E ✅ closed (F1 server + F2 OAuth + F3 secret storage + F4 chat tool + F5 eval runner). **Boundary audit:** `src/main.ts`, `electron/database.cjs`, workbench layout untouched across all 18 commits; only `electron/main.cjs` (+126) and `electron/preload.cjs` (+29) modified for the two user-approved IPC handler families (`shell:openExternal` + `secret:set/get/delete`) — shapes match §T6.F2/F3 spec verbatim. **Storage audit:** zero new ad-hoc `localStorage` writes in M60-touched code; the one new `localStorage` reference in `src/built-in/canvas/properties/propertyBar.ts` (T4 ε) is a sync read-cache for the registry-canonical `canvas.propertyBar.collapsed` setting and is mirrored from `reg.setValue` (matches §3.8 settings registry contract). **Feature-flag audit:** all 8 §3.8 flags registered — `autonomy.followup.enabled`, `autonomy.heartbeat.enabled`, `autonomy.cron.enabled`, `autonomy.subagent.enabled`, `canvas.blockIds.enabled`, `indexing.worker.enabled` (in `autonomySettingsSchemas.ts` + `autonomyFeatureFlags.ts`); `mcp.gmail.enabled` + `mcp.gmail.clientId` + `mcp.gmail.clientSecret` (in `src/built-in/chat/main.ts:371`). One drift documented: §3.8 names the lazy-indexing flag `indexing.lazy.enabled` but the implementation shipped as `indexing.lazyMtime.enabled` (renamed during B5 since it specifically gates the page-mtime fast-skip path, not all lazy indexing — see Phase θ B5 row). **Documentation audit:** all 5 required docs present — `docs/ai/AUTONOMY_TASK_RAIL.md`, `docs/ai/GMAIL_MCP_INTEGRATION.md`, `docs/SETTINGS_REGISTRY.md`, `docs/STARTUP_PERFORMANCE.md`, `tests/autonomy-eval/README.md`. **Deferred items:** (1) §10 plan said "DO NOT update AI_USER_GUIDE.md until T5 rail polish lands"; T5 landed in ζ but `docs/ai/AI_USER_GUIDE.md` was not amended with an autonomy-rail user-facing section — deferred to M61. (2) A1 (legacy `openclawToolLoopSafety.ts` shim deletion) — never staged; the file remains a no-op shim, does not block any wiring; sweep on a future cleanup pass. (3) §3.11 DoD checklist template at line 240 was never instantiated per-domain; the §18 status log Audit/Implementation/Verification/CLOSED rows substitute for explicit DoD ticks (codebase convention — same shape as M55–M59). **Status log audit:** every phase has full coverage — α (Audit/Impl/Wiring/Verif/CLOSED), β (Impl/Verif/CLOSED — audit folded into Implementation row), γ (Audit/Impl/Verif/CLOSED), δ (Audit/Impl/Verif/CLOSED), ε (Audit/Impl/Verif/CLOSED), ζ (Audit/Impl/Verif/CLOSED), η (Audit + per-F1/F2/F3/F4/F5 Impl+Verif + CLOSED), θ (per-B3/B4/B5 Impl+Verif + CLOSED). No orphaned rows. |
