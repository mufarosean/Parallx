...
- **Private sessions (2026-09-11, on request).** The user can ask the assistant to browse privately: `browserOpen` takes `private: true`. That opens a private session, a tab in the chat's own in-memory partition (no `persist:`, so nothing reaches the disk) with every rule of the assistant's profile: the seal, private-address refusal, shields, permissions, downloads to the run folder, and pause on input.
  - It shares nothing with the assistant's usual profile or the user's tabs.
  - Pages it opens stay private, and later opens without `private` stay in it.
  - When its last private tab closes, the partition is wiped, and the next private session gets a new one. Clear Assistant Browser Data clears private sessions too.
  - The tab is titled Private Assistant Browser and carries the Private chip.
  - Evidence: probe gate `privateSession` (a cookie set in the usual profile is not seen in the private tab, and the private tab's cookie is gone in the next private session), plus a runtime unit test.
- **Captures are erased, not deleted (2026-09-11, on request).** The assistant's kept captures and run downloads (`userData/browser/artifacts/<workspace>/<run>/`) are removed in three cases: after 7 days, when their chat is deleted, or on Clear Assistant Browser Data. Removal is always an erase, never the Recycle Bin. Each folder is first renamed out of reach, so no capture id resolves inside it again. It then goes to Eraser when the delete policy names it (`files.eraserPath`); without Eraser, the broker overwrites every file with random bytes before deleting it. An SSD can still hold remapped copies; Eraser's Erase Unused Space covers that (docs/BROWSER.md, "Deleting data"). A private session's captures are kept in memory only, never on disk, and are wiped when the session ends or its data is cleared. Unit tests cover the overwrite (seen through a hard link after the delete), the hand-off to Eraser, and memory-only private captures.
- **Pixel check and overlay scrollbars.** The review's pixel check (`click_at` refuses when the pictured region changed) sometimes refused valid clicks near the right edge. The assistant's own first click moves the mouse, which fades in the overlay scrollbar, and the scrollbar's pixels counted as page changes. The check now leaves out an 18-DIP band along the right and bottom edges, where the browser draws its scrollbars. The rest of the region is still compared, and the stale-content gates still refuse. Evidence: `clickAtZoom` passed 3 of 3 isolated runs, and the full probe passed 65 of 65 (`regression-mtx1rn4h`).
...
- **Images, downloads and moving parts (2026-09-11, from a live report).** The user asked the assistant to download an animated confetti GIF. The assistant kept screenshotting instead: it had no way to name an image, and each new capture differed from the last because the GIF had moved on, so every `click_at` read as a changed page. Three changes:
  - **Images are targets.** `browserRead` lists named images (role `image` or `img` with alt text), up to 20 per page, beside the interactive elements. Unnamed images stay out, so decorative art doesn't crowd the list.
  - **Download, then save.** `browserAct` gains `download`: it fetches the image's own source (`currentSrc`, then `src`) or the link that wraps it into the run's downloads folder. It refuses anything that isn't http(s) and any private address. `save_download` copies a finished download, picked by file name, into the user's Downloads folder, under a new name when the name is taken. The copy shows in the download list as saved by the assistant. The tool description tells the model to use these, never a screenshot, when the user wants a picture or file.
  - **The pixel check ignores what moves on its own.** Right after a capture the broker takes four more looks at uneven gaps (70, 90, 110 and 140 ms). It compares each look with the capture and with the look before, and marks every 8-pixel block that changed as moving. At click time, if the pictured region looks changed, two more looks (90 and 110 ms apart) find blocks that are still moving. The check then compares only blocks that stood still in every look. A real change, such as a relabelled button, is the same in every look and still refuses. When more than 30% of the view moved, the capture itself was a stale frame (seen after a zoom change), so the capture refuses as stale and retries up to three times. Uneven gaps matter: with even gaps an animation on a steady frame rate can show the same frame at every look and pass as still. The probe caught this with an 80 ms flasher.
  - Evidence: probe gates `animatedClickAt` (a click beside a flashing box and on it succeeds, and a relabelled button beside it still refuses) and `imageDownload` (read finds the image, download plus save_download puts `confetti.gif` in Downloads). Unit tests cover the block mask, the static-pixel comparison, save_download, and the capture retry. Test-page elements must fit in about 600 CSS px: past the 653 px view at 150% zoom they add a horizontal scrollbar that moves the edge target. Results: the full probe passed 67 of 67 twice (`regression-mtx3qti5`, `regression-mtx3tp6c`), and the unit suite passed 6,189 tests in 384 files.
- **Captures go with their tab (2026-09-11, the user's rule).** A capture is erased as soon as the Assistant Browser tab it came from closes, whether the user or the assistant closes it. Each capture records its tab. When that tab closes, the file is renamed out of reach and erased (Eraser as a single file, or a random overwrite and delete here). A private capture's bytes are zeroed. The run's in-memory copy for `click_at` is dropped, and the chat's thumbnails re-check and show "Capture erased" (the note used to say "Capture expired"). No tab outlives the app, so at start the broker erases every capture still on disk and finishes any erase that a quit or crash left half done (`*.erase-*`). Downloads are not captures: they stay with their run folder, so `save_download` still works after the tab closes. Run folders keep the 7-day rule, chat deletion and Clear Assistant Browser Data.
- **Pre-commit review (2026-09-11).** Before the commit, four independent reviewers covered the uncommitted work: capture erasure, races in that code, the image, download and pixel-check work, and private sessions with the vision fix. A skeptic then tried to refute each finding. 11 held and 1 was refuted. The refuted one was that closing the window leaves captures behind; in fact the workbench closes the Browser's tabs before the window goes. All 11 are fixed:
  - **A running turn kept an erased capture.** The loop holds its own copy of each capture for the model. The broker now names the erased ids (`artifacts-cleared` carries `artifactIds`). The browser service records them (`src/services/toolImageLifetime.ts`), and the turn loop drops those images before its next model call and never adds one that is already gone.
  - **A test launch could erase a live app's captures.** In test mode, Parallx skips the single-instance lock and can share the dev instance's data folder. A launch like that no longer erases leftovers at start (`eraseLeftoversAtStart`).
  - **The probe was timing the machine's Eraser.** Eraser is installed here, so the probe's captures went to it. The probe now clears `files.eraserPath` for its own workspaces, which makes `captureErasedOnClose` test the broker's own erase.
  - **A locked file stayed readable.** If Windows holds a capture file so the rename fails, its id is still tombstoned (always `ARTIFACT_EXPIRED`). The erase retries in-process every 30 seconds and on each sweep, so an Eraser task that never runs is covered too.
  - **Private sessions leaked to disk.** Blocked trackers from a private assistant session were written to the tracker log and `blocked-stats.json`; now they raise only the lifetime total, as the user's private tabs do. Private downloads now go to a `private-downloads` folder. They are erased file by file when the session ends, and at start if a crash left any. `save_download` says plainly when a file is gone.
  - **Private links went to the regular profile.** Open Link In New Tab, Open Image In New Tab, Search For and New Tab from a private assistant tab now open a private tab.
  - **The click check could be fooled by motion.** Something still moving over the target at click time (a slide, a dialog fading in) could hide itself as motion. Motion first seen at click time may no longer cover the click point's own block or more than 30% of the region (`targetMovesOnlyNow`).
  - **Mostly-moving pages could not be captured.** A page that is mostly video or animation used to fail every capture. The last try now returns the capture without a motion map, so `click_at` compares strictly, and the summary says much of the page is moving.
  - **The screen edges were never checked.** The scrollbar band is now left out only along an edge the page scrolls past (from `cssContentSize`), so the bottom edge of a page that doesn't scroll sideways is checked again.
  - **Smaller fixes.** A cleared workspace folder being erased is skipped by the sweep. Thumbnails re-check only the closed tab's captures. The model-info race timer is cleared.
  - **Known gap, not new.** A download that redirects to a private address is checked only on its first URL, the same gap `browserOpen` has.
  - **Results.** The full probe passed 68 of 68 twice (`regression-mtx6prka`, `regression-mtx6sztc`). `captureErasedOnClose` ran through the broker's own erase, with no Eraser task in the log. The unit suite passed 6,202 tests in 384 files, and `tsc` and `npm run build` are clean.
