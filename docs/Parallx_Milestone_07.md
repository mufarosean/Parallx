...
- **Nested split creation parity:** block handle `Turn into` now includes `2 columns / 3 columns / 4 columns`, replacing the targeted block with a `columnList` in-place (original block preserved in first nested column) to support split-within-split workflows.
- **Action-menu pointer tolerance:** drag-handle action menu no longer closes on transient `column-resize-hover` states from slight sideways pointer movement; it now closes only on active resize drag (`column-resizing`).
- **Image-adjacent handle reliability:** editor mouseout handling now preserves handle/menu interaction when transitioning from content to handle/menu UI, preventing image blocks from causing nearby block handles to disappear before click.
- **Image top alignment parity:** image block vertical margins normalized to the same block rhythm so image tops align with neighboring colored/text block starts.
- **Divider block parity:** thin `horizontalRule` nodes are now explicitly resolved in handle targeting so dividers reliably expose block affordances (`drag handle` + `+`) at top-level and inside columns.
- **Divider row centering:** divider styling now uses an explicit row height with the rule line centered vertically, aligning `drag handle` and `+` to the divider row center.
- **Handle-vs-bubble arbitration:** drag-handle pointer gestures now set a short interaction lock consumed by the bubble menu, preventing accidental inline-format toolbar popups during handle click-drag interactions.
- **Universal handle-resolution parity:** block-handle targeting now resolves via page-container DOM→document mapping instead of selector whitelists, so nested/custom atom blocks (image, media, bookmark, TOC, math, toggle content, tables, etc.) use the same targeting logic as text blocks; `columnList` targets normalize to first visible block in first column per interaction rules.
- **Column-collapse consistency on drag-out:** source-column cleanup now treats placeholder-only remnants as empty after a drag move; when the last meaningful block is moved out, that split column is removed and sibling column widths are redistributed (including nested split rows).
...
- ✅ **Slash media/bookmark/file insertion reliability (February 18, 2026):** slash actions for `Bookmark`, `Video`, `Audio`, and `File` now insert their block nodes immediately (placeholder-first) instead of depending on modal `prompt()` input, so selecting these items consistently creates functional blocks in Electron; regression coverage added in `tests/e2e/12-columns.spec.ts`.
