...
| --- | --- | --- |
| Ads and trackers blocked | EasyList, EasyPrivacy, Peter Lowe and uBlock lists through the Ghostery adblocker engine (MPL-2.0), network filters in `webRequest`; cosmetic CSS through the engine's frame preload; scriptlets (the YouTube-class defence) injected at document start in the main world through the devtools protocol, before any page script and immune to page CSP, chosen per destination when a navigation starts. The protocol link is attached on first need (a destination with scriptlets, a page theme, the assistant driving the tab) and never ahead of it, so a plain page holds no link at all. Counted per tab, shown in the shield | `electron/browserBridge.cjs` |
| Third-party cookies blocked | `Cookie` stripped from third-party requests, `Set-Cookie` stripped from third-party responses; third-party = different registrable domain (tldts) from the tab's document. One exception, the one Chrome and Brave make: a cookie a third party marks `Partitioned` (CHIPS) belongs to that embedding site alone and may be set; Chromium stores it under the top-level site. The wire cannot tell it apart on the way out, so the bridge keeps a ledger of the names each third party set Partitioned under each top-level site and lets only those through in `Cookie`. Block All on a site sends and stores none | bridge, `browserPolicy.cjs` |
| Tracking links cleaned | Brave's debouncing and query filtering, in short form (`debounceTarget`, `stripTrackingParams`), on top-level navigations before any connection. A click-logging link (google.com/url?q=, l.facebook.com/l.php?u=, out.reddit.com, duckduckgo.com/l/, bing.com/ck/a, r.search.yahoo.com, youtube.com/redirect, and a dozen more) goes straight to its destination. A click identifier that ties the visit to an ad or a mailing (`fbclid`, `gclid`, `msclkid`, `mc_eid`, `mkt_tok`, `_hsenc`, and the rest of the known set) is dropped. `utm_*` stays: it names a campaign, not a person, and Brave and Firefox both leave it. Same-site navigations are left alone | bridge, policy |
...
| Honest user agent | The session sends Electron's own user agent (Chrome, Electron and Parallx tokens, as Electron builds it), `Accept-Language` fixed, `Sec-GPC: 1` sent (Global Privacy Control, as Brave does). Until 2026-09-16 the UA was rewritten to plain Chrome with no Electron or app token, on the theory that it hid less. It hid nothing (the page can read the engine a dozen other ways) and it broke every Cloudflare challenge: a Chrome UA arriving with no client hints reads as a spoof, the challenge fails with 600010 and the page reloads into the same challenge. Two other Electron browsers hit the identical loop and fixed it the same way (stablyai/orca PR 18749, pingdotgg/t3code PR 7110). Cloudflare's own troubleshooting names a user agent that changes during a session as a cause of loops | bridge |
| Popups blocked | Chromium's popup blocker is not in Electron, so the rule is rebuilt on the notion it rests on, transient user activation (`activationInput`, `popupDecision` in policy): a page has it for 5 seconds after a mouse press or a key press in it (a release, Escape or a lone modifier grant nothing), one window per activation, never to a destination the lists would stop it embedding (the popunder networks). An allowed one becomes a tab; a blocked one counts in the shield and the tracker log as a popup; non-web schemes are dropped | bridge, policy |
| Redirects judged | A navigation the page itself starts that leaves the site (`navigationDecision`, on `will-navigate`; the address bar, Back and server redirects are not the page's doing) is one of four things. A click (activation, nothing spent): allowed. A tab-under (the activation already spent on a window this page opened): blocked, counted in the shield and the tracker log as a redirect. A fresh document's own redirect (no activation, under 3 seconds since it committed: sign-in hops, shorteners, consent pages): allowed. A settled page sending the user elsewhere with no act of theirs: held, and a bar above the page offers Continue, Stay, or Always Allow Here (the per-site `redirects` setting, also a switch in the shield panel). `browser.holdRedirects` off lets that last case through; tab-unders are blocked either way. The assistant's pages are exempt: the broker consents to each of their navigations itself | bridge, policy, extension |
...
| Links from the app open here | Every web link clicked anywhere in Parallx (pages, notes, chat, PDF annotations, window.open) reaches main.cjs as shell.openExternal or a window-open request; main asks the bridge first, and while the extension is enabled and browser.openLinksInApp is on (default), an http(s) URL becomes a regular tab here. mailto and everything else go to the OS; Open In System Browser passes { system: true } and always does; deactivating the extension withdraws the claim. | main + bridge + policy.inAppLinkDecision |
...
- Challenge pages: tests/probes/browser-turnstile-probe.cjs loads one URL under a ladder of the bridge's configuration (plain Electron, the generic UA, the header edits, the devtools link, the filter engine) and names the first rung that fails a Cloudflare challenge. Diagnostic, not a pass/fail gate.
