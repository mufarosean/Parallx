# Parallx Documentation Index

This folder contains both canonical product docs and historical snapshots.

## Read This First
- Use the files in this root `docs/` folder as canonical unless a section explicitly says otherwise.
- Treat milestone files as implementation history; they may contain intentionally unchecked future tasks.
- Treat `docs/archive/` as historical context, not current source-of-truth.

## Core Architecture
- [ARCHITECTURE.md](../ARCHITECTURE.md)
- [FULL_AUDIT_REPORT.md](./FULL_AUDIT_REPORT.md)
- [NOTION_VS_PARALLX_GAP_ANALYSIS.md](./NOTION_VS_PARALLX_GAP_ANALYSIS.md)

## Canvas Models & Rules
- [CANVAS_STRUCTURAL_MODEL.md](./CANVAS_STRUCTURAL_MODEL.md)
- [BLOCK_INTERACTION_RULES.md](./BLOCK_INTERACTION_RULES.md)
- [PARALLX_WORKSPACE_SCHEMA.md](./PARALLX_WORKSPACE_SCHEMA.md)

## Milestones
- [Parallx_Milestone_01.md](./Parallx_Milestone_01.md)
- [Parallx_Milestone_02.md](./Parallx_Milestone_02.md)
- [Parallx_Milestone_03.md](./Parallx_Milestone_03.md)
- [Parallx_Milestone_04.md](./Parallx_Milestone_04.md)
- [Parallx_Milestone_05.md](./Parallx_Milestone_05.md)
- [Parallx_Milestone_06.md](./Parallx_Milestone_06.md)
- [Parallx_Milestone_07.md](./Parallx_Milestone_07.md)
- [Parallx_Milestone_08.md](./Parallx_Milestone_08.md)
- [Parallx_Milestone_40.md](./Parallx_Milestone_40.md)

## Research
- [research/tiptap-columns-research.md](./research/tiptap-columns-research.md)
- [research/WORKSPACE_CANVAS_PERSISTENCE_RESEARCH.md](./research/WORKSPACE_CANVAS_PERSISTENCE_RESEARCH.md)
- [research/README.md](./research/README.md)
- [ai/M40_AI_ENTRYPOINT_INVENTORY.md](./ai/M40_AI_ENTRYPOINT_INVENTORY.md)
- [ai/M40_PHASE1_STRESS_BASELINE.md](./ai/M40_PHASE1_STRESS_BASELINE.md)
- [ai/M40_PHASE1_CANVAS_BASELINE.md](./ai/M40_PHASE1_CANVAS_BASELINE.md)

## Archive
- [archive/FULL_AUDIT_REPORT.md](./archive/FULL_AUDIT_REPORT.md)
- [archive/PHASE1_CLEANUP.md](./archive/PHASE1_CLEANUP.md)
- [archive/FEB18_2026_CANVAS_INTERACTION_HARDENING.md](./archive/FEB18_2026_CANVAS_INTERACTION_HARDENING.md)
- [archive/README.md](./archive/README.md)

## Canonical vs Historical
- Canonical audit report: [FULL_AUDIT_REPORT.md](./FULL_AUDIT_REPORT.md)
- Historical audit snapshot: [archive/FULL_AUDIT_REPORT.md](./archive/FULL_AUDIT_REPORT.md)
